...
// SIGNIFICANT_SHARE and another track's share in that same band is also
// above SIGNIFICANT_SHARE, cut the other track by CUT_DB in that band.
 ...
 */
export function detectMasking(tracks, referenceTrack) {
  ...
  for (const track of tracks) {
    ...
    for (let i = 0; i < bands.length; i++) {
      ...
      if (refBand.share >= SIGNIFICANT_SHARE && trackBand.share >= SIGNIFICANT_SHARE) {
